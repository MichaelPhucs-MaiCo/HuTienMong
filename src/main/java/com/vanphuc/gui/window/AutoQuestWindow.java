...
public class AutoQuestWindow extends Window {
    ...
    public void refresh() {
        lines.clear();
        if (targetList.getValue().isEmpty()) {
            lines.add(new StringBuilder());
        } else {
            for (String s : targetList.getValue()) {
    ...
    public void draw(DrawContext context, float partialTicks) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            // Vẽ bôi đen màu Xanh Blue Sleek Carbon
