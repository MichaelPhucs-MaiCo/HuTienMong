package com.vanphuc.gui.navigation.friends;
...
public class FriendWindow extends Window {
    ...
    public FriendWindow(Rectangle position) {
        super("§lDanh sách Bạn bè", position);
        refresh();
    }
    ...
    public void draw(DrawContext context, float partialTicks) {
        this.position.setHeight(Math.max(150f, titleHeight + 16f + (lines.size() * 12f)));
    ...
    private void saveData() {
        ...
        // Tự động clear mấy dòng trống để nhìn cho gọn
        for (StringBuilder sb : lines) {
            if (!sb.toString().trim().isEmpty()) {
                saved.add(sb.toString().trim());
            }
        }
        FriendManager.setFriends(saved);
        ConfigManager.save();
    }
}