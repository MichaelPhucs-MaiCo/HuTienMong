...
public class AutoSwitchHotbarHud extends HudWindow {
    ...
    public void draw(DrawContext context, float partialTicks) {
        ...
        List<AutoSwitchHotbar.Task> tasks = module.getTasks();

        // 1. Tự động co giãn chiều cao (Scale height) dựa theo số lượng slot
        int taskCount = !tasks.isEmpty() ? tasks.size() : 1;
        ...
        if (!tasks.isEmpty()) {
            for (AutoSwitchHotbar.Task task : tasks) {
                ...
                if (module.isActive()) {
                    if (module.isStartupPhase()) {
                        ...
                        timeColor = (remainingSec <= 3) ? 0xFFFF5555 : 0xFFF1C40F;
                    }
                } else {
                    // Khi module tắt, hiển thị cấu hình với màu xám tro
                    timeText = "§l" + (task.delayMs / 1000) + "s";
